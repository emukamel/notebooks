...
This script is used to convert segmetn format. The input format is with rich information which also
include patterns with missing info (such as ??11). The output is only for patterns with full information,
and also shows the calculated ML and ME for each segment.

=head1 Usage

perl convert_seed_format.pl <detailed seed file>  > new_seed_file
